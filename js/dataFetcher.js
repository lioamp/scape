...
export async function fetchPlatformData(platform) {
    ...
    try {
        if (platform === 'all') {
            console.log("Fetching data for All Platforms (TikTok & Facebook)...");
        ...
        console.log(`Normalized ${platform} raw data fetched:`, normalizedData);
...
export async function fetchSalesChartData(timeRange) {
    ...
    try {
        console.log("Fetching sales chart data from URL:", url);
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
        const data = await response.json();
        console.log("Raw sales chart data from API:", data); 
